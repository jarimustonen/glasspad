...
`glasspad skill install` currently installs this CLI's companion skill(s) into
...
`glasspad skill install` bypass `dotfiles link`, so their skills stay Claude-only
...
- `glasspad skill install` (and `--force` / `--agent` variants) create each skill
