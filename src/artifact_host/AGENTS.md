...
- `headers.rs` — the two header sets. `artifact_csp()` = sandbox + egress CSP
  that **names the explicit host** (`'self'` is meaningless under a null
  origin); `shell_csp()` = trusted-chrome CSP (nonce'd script + Trusted Types).
...
- `shell.rs` — the trusted parent document. Runs the **parent side of the
  postMessage bridge**: validates `event.source === iframe.contentWindow` (NOT
  `event.origin`, which is `"null"` for every sandboxed frame), caps byte-size +
  rate, accepts only `{type:"navigate", slug:<known>}`, inserts artifact text as
  `textContent` (never `innerHTML`).
- `guards.rs` — control-plane guards (design.md §5): `host_guard` (DNS-rebinding
  defense, all routes) + `control_origin_guard` (reject `Origin: null`/foreign on
